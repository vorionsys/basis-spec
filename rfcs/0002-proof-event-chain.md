...
interface ProofEvent {
  ...
  signature?:     string;            // detached Ed25519 signature over the CANONICAL BYTES
                                     //   (the same bytes eventHash is computed from —
                                     //    NOT over the eventHash hex string; see Erratum E-1)
...
- **Public reference verifier (shipped):** [`@vorionsys/basis-spec-conformance`](https://www.npmjs.com/package/@vorionsys/basis-spec-conformance) ≥ 0.2.0 exports `verifyChain()` and a `basis-conformance verify` CLI. It walks any chain, recomputes the canonical-JSON bytes, re-derives `eventHash` (and `eventHash3` when present), checks `previousHash` linkage from a null head, verifies detached Ed25519 signatures, and returns a superset of `ChainVerificationResult`. It depends on nothing beyond `node:crypto`, so an auditor can read it end to end.

  ```bash
  npx @vorionsys/basis-spec-conformance verify chain.json --keys keys.json
  ```

  Golden vectors ship in the package under `vectors/` — a valid signed chain plus tampered-payload, cut-linkage, bad-head, corrupted-signature, and signature-domain-mismatch variants. They are generated by an *independently written* canonicalizer (`scripts/generate-vectors.mjs`), so a passing run proves two separate readings of §"Canonical serialization" agree on the bytes.

  The verifier reports on the **integrity of the record only**. A cryptographically perfect chain of bad decisions verifies just fine — that is what a receipt is, and it is not a trust or compliance verdict about the agent.

- **⚠️ `@vorionsys/verify` (bin `basis-verify`) does NOT verify RFC-0002 chains today.** Its npm description reads "Offline verifier for BASIS proof chains", but **verified 2026-08-02 against v0.6.0**: `--format basis` requires an envelope `{ "basisVerify": "1", "records": [ … ] }` whose records carry `seq`, `ts`, `action`, `prev_hash`, `pubkey`, `key_id`, `sig` — a different schema from the one specified above. A conforming RFC-0002 chain fails at record 0 with `missing fields`. Both of this repo's golden vector sets were rejected.

  This matters for [RFC-0004](0004-hash-suite-convergence.md), whose §3 ("One verifier") treats `@vorionsys/verify` as already handling the BASIS family and needing only to learn the Aurais suite. On the evidence above it handles *neither* RFC-0002 family, so that acceptance criterion is larger than it appears. Convergence work should start from a measured schema comparison rather than from the assumption of a shared base.
...
### Erratum E-1 (2026-08-02): what the signature covers

The v1.0/v1.1 text was **self-contradictory** about the signed message. §"Schema" annotated `signature` as "detached signature over eventHash", while §"Verification procedure" step 5 said to verify "the detached signature over the canonical bytes". Those are different messages: a signature valid under one reading is invalid under the other, so two conforming-by-the-letter implementations could fail to verify each other's chains — exactly the interop failure this RFC exists to prevent.

**Resolution: the signature covers the CANONICAL BYTES** — the same UTF-8 byte string that `eventHash` is computed over. The §"Verification procedure" reading is normative; the §"Schema" annotation was wrong and has been corrected.

Rationale: signing the canonical bytes binds the signature to the event's actual content. Signing the hex `eventHash` string binds it only to a digest the signer may have received rather than computed, which weakens the guarantee for no benefit — the hash is recomputed during verification anyway.

The reference verifier defaults to `canonical`, accepts `signatureDomain: 'eventHash'` for chains sealed under the old ambiguous reading, and when a signature fails under the active domain it retries under the other and reports `domain-mismatch` rather than a bare "bad signature", so implementers get an actionable diagnostic. A golden vector (`chain-signature-domain-mismatch.json`) pins this behavior.
